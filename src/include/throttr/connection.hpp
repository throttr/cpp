...
class connection : public std::enable_shared_from_this<connection>
{
  public:
    ...
    connection(const boost::asio::any_io_executor& executor, std::string host, uint16_t port)
        : strand_(make_strand(executor)), resolver_(strand_), socket_(strand_),
                ...
                const boost::system::error_code& ec, const auto& endpoints) mutable
            {
                ...
                boost::asio::async_connect(socket_,
                                           ...
                                           [self, _final_handler = std::move(_scope_handler)](
                                               ...
                                               auto) mutable { _final_handler(connect_ec); });
            });
    };
              ...
              std::function<void(boost::system::error_code, std::vector<std::byte>)> handler)
    {
        ...
        post(strand_,
             [_self,
              _scoped_buffer = std::move(buffer),
  ...
  private:
                          ...
                          const std::shared_ptr<std::array<std::byte, 1>>& head)
    {
        ...
        boost::asio::async_read(
            ...
            boost::asio::bind_executor(
                strand_,
                [_self, operation, head, _rest](const boost::system::error_code& ec, std::size_t)
                {
                    if (ec)
                    {
                        operation->handler(ec, {});
     ...
     * @param operation
     */
    void handle_response_status(const std::shared_ptr<write_operation>& operation)
    {
        auto _self            = shared_from_this();
