...
namespace throttr {
    ...
    private:
        ...
        void handle_write(const std::shared_ptr<write_operation> &operation) {
            ...
            auto continuation = [_self, operation](boost::system::error_code ec, std::vector<std::byte> response) {
                if (ec) {
            ...
            switch (_type) {
                case 0x02: {
                    const auto head = std::make_shared<std::array<std::byte, 1> >();
                    (*head)[0] = std::byte{_type};
                    read_query_value(operation, head, std::move(continuation));
                    break;
                }
                case 0x06: {
                    auto head = std::make_shared<std::array<std::byte, 1> >();
                    (*head)[0] = std::byte{_type};
                    read_get_header(operation, head, std::move(continuation));
                    break;
                }
                default:
                    read_status_value(operation, std::move(continuation));
                    break;
            }
        }

        void read_status_value(
            const std::shared_ptr<write_operation> &op,
            std::function<void(boost::system::error_code, std::vector<std::byte>)> next) {
            auto self = shared_from_this();
            auto buf = std::make_shared<std::vector<std::byte> >(1);

            boost::asio::async_read(
                socket_, boost::asio::buffer(*buf),
                boost::asio::transfer_exactly(1),
                boost::asio::bind_executor(strand_,
                                           [self, op, buf, next](boost::system::error_code ec, std::size_t) {
                                               if (ec) {
                                                   next(ec, {});
                                               } else {
                                                   next({}, std::move(*buf));
        ...
        void read_get_header(const std::shared_ptr<write_operation> &operation,
            ...
            boost::asio::async_read(
                ...
                boost::asio::bind_executor(
                    strand_, [_self, operation, head, _header, next](
                const boost::system::error_code &ec, std::size_t) {
                        // LCOV_EXCL_START
                        if (ec) {
                            next(ec, {});
                        ...
                        _self->read_get_value(operation, head, _header, _size, std::move(next));
                    }));
        }
        ...
        void read_query_value(const std::shared_ptr<write_operation> &operation,
            ...
            boost::asio::async_read(
                    ...
                    boost::asio::transfer_exactly(1),
                    boost::asio::bind_executor(strand_,
                        [_self, operation, success_byte, next](const boost::system::error_code &ec, std::size_t) {
                            if (ec) {
                                next(ec, {});
                                return;
                            }
                            ...
                            // Si success == 0x01, seguimos leyendo: quota (4), ttl_type (1), ttl (4)
                            ...
                            boost::asio::async_read(
                                ...
                                boost::asio::bind_executor(_self->strand_,
                                    [success_byte, rest, next](const boost::system::error_code &ec2, std::size_t) {
                                        if (ec2) {
                                            next(ec2, {});
                                            return;
                                        }
