...
TEST_F(TcpConnectionTest, ConnectWaitAndDisconnect) {
  ...
  ASSERT_NO_THROW({ boost::asio::connect(socket_, _endpoints); });
...
TEST_F(TcpConnectionTest, ConcatenateTwoInsertsAndQueriesAtOnce) {
  ...
  const auto _buffer3 =
      throttr::request_query_builder("consumer:batch|api/test-1");
  const auto _buffer4 =
      throttr::request_query_builder("consumer:batch|api/test-2");

  std::vector<std::byte> _concatenated;
  _concatenated.reserve(_buffer1.size() + _buffer2.size() + _buffer3.size() +
                        _buffer4.size());
  _concatenated.insert(_concatenated.end(), _buffer1.begin(), _buffer1.end());
  _concatenated.insert(_concatenated.end(), _buffer2.begin(), _buffer2.end());
  ...
  boost::system::error_code _ec;
  const size_t _n =
      boost::asio::read(socket_, boost::asio::buffer(_response), _ec);
